...
router.get('/modifications', async (req: Request, res: Response) => {
  try {
    ...
    if (status) where.approvalStatus = String(status);
...
router.get('/modifications/:id', async (req: Request, res: Response) => {
  try {
...
router.patch('/modifications/:id/decision', async (req: Request, res: Response) => {
  try {
    const { approvalStatus, customerResponse } = req.body as {
...
router.post('/modifications/scan', async (_req: Request, res: Response) => {
  try {
    ...
    const flags: Array<{ jobId: string; reason: string; severity: 'LOW' | 'MEDIUM' | 'HIGH' }> = [];
