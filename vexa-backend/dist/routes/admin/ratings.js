...
router.get('/ratings', async (req, res) => {
    try {
        ...
        const where = {};
        if (score)
            where.overallScore = Number(score);
        if (raterId)
            where.raterId = String(raterId);
        if (rateeId)
            where.rateeId = String(rateeId);
        if (jobId)
        ...
        const ratingsWithReviews = await Promise.all(items.map(async (item) => {
...
router.get('/ratings/overview', async (_req, res) => {
    try {
...
router.delete('/ratings/:id', requireAdminRole(['SUPER_ADMIN']), async (req, res) => {
    try {
        const existing = await prisma.rating.findUnique({ where: { id: String(req.params.id) } });
        if (!existing) {
            res.status(404).json({ success: false, message: 'Rating not found' });
