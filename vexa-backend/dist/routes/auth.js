...
import { captchaMiddleware, isCaptchaConfigured } from '../lib/captcha';
...
import { validateRegistration, validateLogin, validateEmail, validatePassword, validatePhone, validateName, } from '../utils/validation';
const router = Router();
...
const toPublicUser = (user) => ({
    ...
    kycDocuments: user.kycVerifications?.map((doc) => doc.fileUrl) || [],
    createdAt: user.createdAt,
    updatedAt: user.updatedAt,
});
const ensureAccountAccess = async (user, res) => {
...
const createUserCompat = async (data) => {
    try {
        ...
        const now = new Date();
        const availableColumns = await prisma.$queryRaw `
      SELECT column_name
      FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'users'
    `;
        const existingColumns = new Map(availableColumns.map((col) => [col.column_name.toLowerCase(), col.column_name]));
        const candidates = [
            ['id', fallbackId],
            ['email', data.email],
            ['name', data.name],
        ...
        await prisma.$executeRawUnsafe(`INSERT INTO "users" (${columnSql}) VALUES (${valuesSql})`, ...insertValues);
...
router.get('/captcha/site-key', (_req, res) => {
...
// ─── POST /api/auth/register ───────────────────────────
router.post('/register', captchaMiddleware('register'), async (req, res) => {
    try {
        ...
        const normalizedProviderSkills = Array.isArray(initialSkills)
            ? Array.from(new Set(initialSkills
                .map((skill) => normalizeServiceCategory(String(skill || '')))
                .filter((skill) => AVAILABLE_SERVICE_CATEGORIES.includes(skill))))
            : [];
        ...
        if (role === 'PROVIDER' && normalizedProviderSkills.length > 0) {
            await prisma.providerSkill.createMany({
                data: normalizedProviderSkills.map((category) => ({
                    providerId: user.id,
...
// ─── POST /api/auth/login ──────────────────────────────
router.post('/login', captchaMiddleware('login'), async (req, res) => {
    try {
...
router.post('/otp/send', captchaMiddleware('otp_send'), async (req, res) => {
    try {
        ...
        // Send email
        sendLoginOtpEmail(user.email, user.name, otpCode).catch((err) => console.error('[Email] Failed to send login OTP:', err));
        res.json({
            success: true,
...
router.post('/forgot-password', captchaMiddleware('forgot_password'), async (req, res) => {
    try {
        const { email } = req.body;
        const emailValidation = validateEmail(email || '');
        if (!emailValidation.valid) {
