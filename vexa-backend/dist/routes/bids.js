...
const router = Router();
const ensureProviderCanBid = async (userId) => {
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        if (providerSkills.length > 0) {
            const hasMatchingSkill = providerSkills.some((s) => s.categoryName.toLowerCase() === (job.categorySlug || 'general').toLowerCase());
            if (!hasMatchingSkill) {
                res.status(403).json({
                    success: false,
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const updated = await prisma.bid.update({
            where: { id: req.params.id },
            data: {
                ...(hasAmountUpdate && { amount: numericAmount }),
                ...(message && { message }),
