...
const getMinimumPriceForCategory = (category) => {
...
const generateOrderIdCandidate = () => {
    const now = new Date();
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const { status, category, page = '1', limit = '20', lat, lng, radiusKm, minPrice, maxPrice, sortBy, } = req.query;
        ...
        if (role === 'CUSTOMER') {
            ...
            if (skillCategories.length > 0) {
                const categoryFilters = skillCategories.map((skillCategory) => ({
                    categorySlug: { equals: skillCategory, mode: 'insensitive' },
                }));
                where.OR = [
                    {
        ...
        if (status)
            where.status = status;
        if (category)
            where.categorySlug = { equals: String(category), mode: 'insensitive' };
        // Price range filters
        if (minPrice || maxPrice) {
            where.originalPrice = {};
            if (minPrice)
                where.originalPrice.gte = Number(minPrice);
            if (maxPrice)
                where.originalPrice.lte = Number(maxPrice);
        }
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            getIO().emit('job:new', jobFormatted);
        }
        catch (e) { }
...
router.patch('/:id/status', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            getIO().emit('job:statusChange', { jobId: req.params.id, status, job: jobFormatted });
        }
        catch (e) { }
...
router.patch('/:id/complete', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            getIO().emit('job:statusChange', { jobId, status: 'COMPLETED', job: updatedFormatted });
        }
        catch (e) { }
...
router.patch('/:id/accept-work', authMiddleware, async (req, res) => {
    try {
        const jobId = req.params.id;
        const job = await prisma.serviceRequest.findUnique({ where: { id: jobId } });
        if (!job) {
        ...
        try {
            getIO().emit('job:statusChange', { jobId, status: 'PAYMENT_PENDING', job: updatedFormatted });
        }
        catch (e) { }
        res.json({ success: true, data: updatedFormatted });
    }
