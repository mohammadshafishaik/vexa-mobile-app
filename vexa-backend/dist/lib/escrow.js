 ...
 */
export async function captureEscrowPayment(razorpayOrderId, razorpayPaymentId, razorpaySignature) {
    try {
 ...
 */
export async function releaseEscrow(escrowId, releasedBy) {
    try {
        const escrow = await prisma.escrowTransaction.findUnique({
            where: { id: escrowId },
 ...
 */
export async function refundEscrow(escrowId, refundAmount, reason, adminId) {
    try {
        const escrow = await prisma.escrowTransaction.findUnique({
            where: { id: escrowId },
        });
 ...
 */
export async function lockForDispute(escrowId, disputeId) {
    try {
        const escrow = await prisma.escrowTransaction.findUnique({ where: { id: escrowId } });
        if (!escrow)
            return { success: false, error: 'Escrow not found' };
        if (escrow.status !== 'HELD')
            return { success: false, error: `Cannot lock escrow in ${escrow.status} status` };
        await prisma.escrowTransaction.update({
            where: { id: escrowId },
            data: {
...
export async function unlockFromDispute(escrowId) {
    try {
        await prisma.escrowTransaction.update({
            where: { id: escrowId },
            data: {
...
// ─── Get Escrow Status ──────────────────────────────────────────────────────
export async function getEscrowStatus(jobId) {
