 ...
 */
export async function registerPushToken(userId, token, platform, deviceId, deviceName) {
 ...
 */
export async function sendPushNotification(userId, title, body, notificationType, data) {
    if (!useFirebase) {
 ...
 */
export async function sendDataMessage(userId, data) {
    if (!useFirebase)
        return false;
    const tokens = await getActiveTokens(userId);
    if (tokens.length === 0)
 ...
 */
export async function sendMulticastNotification(userIds, title, body, notificationType, data) {
    if (!useFirebase || userIds.length === 0)
        return 0;
