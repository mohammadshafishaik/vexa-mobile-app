...
// src/lib/prisma.ts
var connectionString = process.env.DATABASE_URL || process.env.DATABASE_INTERNAL_URL || process.env.POSTGRES_URL || process.env.POSTGRES_PRISMA_URL || process.env.RENDER_DATABASE_URL || (process.env.NODE_ENV !== "production" ? "postgresql://shaikshafi@localhost:5432/vexa_db" : void 0);
if (!connectionString) {
...
var SMTP_USER = process.env.SMTP_USER || "app.vexa.in@gmail.com";
var SMTP_PASS = process.env.SMTP_PASS || "";
var FROM_EMAIL = SMTP_USER;
var FROM_NAME = process.env.EMAIL_FROM_NAME || "VEXA";
var APP_URL = process.env.APP_URL || process.env.BETTER_AUTH_URL || "http://localhost:3000";
...
if (SMTP_USER && SMTP_PASS) {
  ...
  console.log(`[Email] Nodemailer SMTP initialized (from: ${FROM_NAME} <${FROM_EMAIL}>)`);
} else {
  console.warn("[Email] SMTP credentials (SMTP_USER/SMTP_PASS) not set \u2014 email sending disabled");
}
async function sendEmail(options) {
  if (!transporter) {
    console.error("[Email] Cannot send: Nodemailer SMTP not configured");
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const isRetryable = err?.responseCode === 429 || err?.responseCode >= 500;
      ...
      console.error(`[Email] Failed after ${attempt} attempt(s):`, err.message);
...
<tr><td style="text-align:center;padding-top:24px;">
  <p style="margin:0;font-size:12px;color:#555;">
    \xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} VEXA. All rights reserved.<br/>
    <a href="${APP_URL}/unsubscribe" style="color:#666;text-decoration:underline;">Unsubscribe</a> \xB7 
...
async function sendLoginOtpEmail(to, name2, code) {
  return sendEmail({
    to,
    subject: "Your VEXA Login OTP \u{1F511}",
    html: baseLayout(`
      <h2 style="color:#fff;margin:0 0 16px;font-size:22px;">Login Verification Code</h2>
      <p style="color:#ccc;line-height:1.7;font-size:15px;">Hi ${name2}, use this One-Time Password (OTP) to log into your VEXA account:</p>
...
async function run() {
  console.log("--- Testing Login OTP Generation and Verification ---");
  const testEmail = "sk.mohammadshafi3044@gmail.com";
  const user = await prisma_default.user.findUnique({
    where: { email: testEmail }
  });
  if (!user) {
    console.error(`User with email ${testEmail} does not exist in the database. Please run seed or register first.`);
    return;
  }
  ...
  await prisma_default.verification.upsert({
    where: { id: identifier },
    create: {
      id: identifier,
      identifier,
  ...
  const verificationRecord = await prisma_default.verification.findUnique({
    where: { id: identifier }
  });
  if (verificationRecord && verificationRecord.value === otpCode && /* @__PURE__ */ new Date() < verificationRecord.expiresAt) {
    console.log("OTP Verification Check: SUCCESS \u2705 (OTP matches and is not expired)");
  } else {
    console.error("OTP Verification Check: FAILED \u274C");
  }
}
run().catch(console.error).finally(() => prisma_default.$disconnect());
